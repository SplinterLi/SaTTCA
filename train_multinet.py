...
from models.unet_multinet import UNet
from loss import SegLoss
from data import transforms as aug
from data.dataset_multinet import MSDataset
...
def _init_dataloaders(dataset):
    ...
    ds_train = MSDataset(cfg.data_base, file_csv, transforms_train, train_set, dataset, 0, cfg.input_shape)
    dl_train = MSDataset.get_dataloader(ds_train, cfg.batch_size,
                                            shuffle=True, num_workers=cfg.num_workers)
    ds_val = MSDataset(cfg.data_base, file_csv, transforms_val, val_set, dataset, 0, cfg.input_shape)
    dl_val = MSDataset.get_dataloader(ds_val, cfg.batch_size, False,
                                          cfg.num_workers)
...
def _train_epoch(model, dataloader, criterion, optimizer, scheduler, niis_dir, epoch):
    ...
    if cfg.multiscale:
        loss_train_dict = []
        dice_train_dict = []
        for i in range(len(cfg.input_shape)):
            loss_train_dict.append(_init_loss_dict())
            dice_train_dict.append(_init_dice_dict())
        len_train_dict = deepcopy(loss_train_dict[0])
        len_train_dice = deepcopy(dice_train_dict[0])
    else:
    ...
    for idx, sample in enumerate(dataloader):
        optimizer.zero_grad()
        if cfg.multiscale == True:
            images = []
            masks = []
            for i in range(len(sample["images"])):
                images.append(sample["images"][i].cuda())
                masks.append(sample["masks"][i].cuda())
        else:
        ...
        if cfg.multiscale:
            loss = sum(loss_dict["total_loss"])
            loss.backward()
        else:
            loss_dict["total_loss"].backward()
        optimizer.step()
        scheduler.step()
        dice_dict = _dice_by_label(masks, outputs, datasets)
        if cfg.multiscale:
            for i in range(len(dice_dict)):
                for k in loss_dict.keys():
                    loss_train_dict[i][k] += loss_dict[k][i].detach().cpu().item()
                    if k != "total_loss" and i == 0:
                        len_train_dict[k] += 1
                for k in dice_dict[i].keys():
                    dice_train_dict[i][k] = np.append(dice_train_dict[i][k], dice_dict[i][k])
                    if k != "dice" and i == 0:
                        len_train_dice[k] += 1
        else:                       
            for k in loss_dict.keys():
            ...
            for k in dice_dict.keys():
                ...
                if k != "dice":
                    len_train_dice[k] += 1
    if cfg.multiscale:
        for i in range(len(dice_train_dict)):
            for k in loss_train_dict[i].keys():
                if k == "total_loss":
                    loss_train_dict[i][k] /= len(dataloader)
                else:
                    if len_train_dict[k] > 0:
                        loss_train_dict[i][k] /= len_train_dict[k]

            for k in dice_train_dict[i].keys():
                dice_train_dict[i][k] = np.mean(dice_train_dict[i][k])
    else:            
    ...
    return loss_train_dict,  dice_train_dict, names, lr, dice_dict
...
def _eval_epoch(model, dataloader, criterion, niis_dir, epoch):
    ...
    if cfg.multiscale == True:
        ...
        dice_val_dict = []
        for i in range(len(cfg.input_shape)):
            loss_val_dict.append(_init_loss_dict())
            dice_val_dict.append(_init_dice_dict()) 
        len_val_dict = deepcopy(loss_val_dict[0])
        len_val_dice = deepcopy(dice_val_dict[0])
    else:
    ...
    for idx, sample in enumerate(dataloader):
        ...
        if cfg.multiscale == True:
            for i in range(len(dice_dict)):
                for k in loss_dict.keys():
                    loss_val_dict[i][k] += loss_dict[k][i].cpu().item()
                    if k != "total_loss" and i == 0:
                        len_val_dict[k] += 1

                for k in dice_dict[i].keys():
                    dice_val_dict[i][k] = np.append(dice_val_dict[i][k], dice_dict[i][k])
                    if k != "dice" and i == 0:
                        len_val_dice[k] += 1
        else:
            for k in loss_dict.keys():
    ...
    if cfg.multiscale ==True:
        for i in range(len(dice_val_dict)):
            for k in loss_val_dict[i].keys():
                if k == "total_loss":
                    loss_val_dict[i][k] /= len(dataloader)
                else:
                    if len_val_dict[k] > 0:
                        loss_val_dict[i][k] /= len_val_dict[k]


            for k in dice_val_dict[i].keys():
                if len(dice_val_dict[i][k]) > 0:
                    dice_val_dict[i][k] = np.mean(dice_val_dict[i][k])
                else:
                    dice_val_dict[i][k] = 0
    else:
    ...
    return loss_val_dict, dice_val_dict, names, dice_dict
...
def _dice_by_label(y_true, y_pred, dataset_list):
    if cfg.multiscale:
        ...
        for i in range(len(y_pred)):
            ...
            y_pred_ = (y_pred[i]> 0).float().cpu().detach().numpy()
            dice_list = dice_by_label(y_true_, y_pred_)
        ...
        y_pred = (y_pred > 0).float().cpu().detach().numpy()
...
def main(dataset):
    ...
    model = _init_models()
    criterion = SegLoss(cfg.multiscale)
    ...
    for i in range(cfg.epochs):
        ...
        if cfg.multiscale:
            for j in range(len(cfg.input_shape)):
                print("input size", 64 / 2 ** j)    
                _log_metrics([loss_train[j]])
                _log_metrics([dice_train_dict[j]])
                _log_tensorboard(tb_writer, i, loss_train[j], 'loss_{}'.format(64 / 2 ** j))
                _log_tensorboard(tb_writer, i, dice_train_dict[j], 'dice_{}'.format(64 / 2 ** j))
        else:    
        ...
        if i % 10 == 9:
            ...
            if cfg.multiscale == True:
                for j in range(len(cfg.input_shape)):
                    print("input size", 64 / 2 ** j)
                    _log_metrics([loss_train[j], loss_val[j]])
                    _log_metrics([dice_train_dict[j], dice_val_dict[j]])
            else:
